...
Runs the autonomous CEO Agent over all retrieved evidence.
No predefined questions. The agent reasons freely.
...
REPORT_PATH   = PROJECT_ROOT / "latest_report.json"
...
def main():
    ...
    print("\n" + "=" * 60)
    print("  Airbus CEO Agent — Autonomous Report Generator")
    print("=" * 60)

    save_progress("Loading corpus…")
    print("\n[1/4] Loading corpus stats…")
    ...
    save_progress("Retrieving evidence across all strategic dimensions…")
    print("\n[2/4] Retrieving & reranking evidence…")

    save_progress("Running strategic analysis…")
    print("\n[3/4] Running CEO Agent (RAG + analysis + Llama 3.1)…")
    print("      Agent is reasoning over evidence autonomously…")

    try:
        from CEOAgent.ceo_agent import ask_ceo_agent
        result = ask_ceo_agent(top_k=args.top_k)

        intel = result["rag_result"]["intelligence"]
        print(f"      ✓ {len(intel['opportunities'])} opportunities · "
              f"{len(intel['risks'])} risks · "
              f"{len(intel['trends'])} trends detected")
        print("      ✓ CEO report generated by Llama 3.1")
